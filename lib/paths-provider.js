...
import { score, filter } from 'fuzzaldrin-plus-fast'
...
export default class PathsProvider extends EventEmitter {
  ...
  _getSuggestionsForScope (scope, request, match) {
    ...
    let suggestions = files.map(pathName => {
      ...
      const imgRegex = new RegExp(`.(png|svg|jpg|jpeg|jfif|pjpeg|pjp|gif|apng|ico|cur)$`)
      const iconHTML = imgRegex.test(absolutePath) ?
